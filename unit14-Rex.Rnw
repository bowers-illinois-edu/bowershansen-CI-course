...
\title{Sensitivity Analysis II: Hosman, Hansen, and Holland Style}
...
\begin{enumerate}
  ...
  \item We have now worked to create matched designs for the Metrocable
    intervention in Columbia. Last time we engaged with a variety of questions
    that emerge from applied work: ill-behaved propensity scores, too many
    matched set indicators for computationally-efficient estimation of
    stratum weighted treatment effects, questions about the appropriateness of
    different proposals for randomization-based/design-based standard errors,
    and precision enhancement using background information after matching
    (also known as ``covariance adjustment'').
...
library(brglm)
brglm1<-brglm(balfmlaBig,data=meddat,family=binomial,control.brglm=brglm.control(br.maxit=500))
...
\item Ok. Now, to continue with the \cite{hosman2010} development. They
  ...
  matching. I present two methods here (the in-progress \texttt{mlm} method
  and the HC2 method):

<<results="hide">>=
source("mlm.R") ## matched set linear model code
source(url("http://jakebowers.org/ICPSR/confintHC.R"))
library(sandwich)
library(lmtest)

mlm1<-mlm(HomRate0803~nhTrt+bestfm,data=meddat,ms.weights=harmonic)
lm1<-lm(HomRate0803~nhTrt+bestfm,data=meddat)

theHC2ci<-confint.HC(lm1,level=.95,parm="nhTrt",thevcov=vcovHC(lm1,type="HC2"))
theAIci<-confint.HC(mlm1,level=.95,parm="x1",thevcov=vcov(mlm1))
@
