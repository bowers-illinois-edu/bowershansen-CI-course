...
\date{August 12, 2022} % Date, can be changed to a custom date
...
\vfill
\begin{figure}
\includegraphics[width = \linewidth]{Identification_Figure.pdf}
\end{figure}
\vfill
\end{frame}
%---------------------------------------------------------------
...
\begin{frame}[t]
\frametitle{DID Estimation}
Parallel trends $\implies$ expectation of DID estimator $=$ ATT 
\begin{itemize}
\item DID estimator's expected value:
\begin{equation*}
\begin{split}
\E\left[\widehat{\text{DID}}_T\right] & = \underbrace{\E\left[Y_{iT}(1) - Y_{iT-1} \given G_i = 1\right] - \E\left[Y_{iT} - Y_{iT-1} \given G_i = 0\right]}_{\text{Descriptive difference between treated and control populations}}
\end{split} 
\end{equation*}
...
\normalsize
\vspace{0.75em}
\item Substitute for $\E\left[Y_{iT} \given G_i = 0\right]$ in DID estimator
\small\begin{align*}
\underbrace{\E\left[Y_{iT}(1) - Y_{iT-1} \given G_i = 1\right] - \E\left[Y_{iT} - Y_{iT-1} \given G_i = 0\right]}_{\text{Descriptive difference between treated and control populations}} \\ = \underbrace{\E\left[Y_{iT}(1) - Y_{iT}(0) \given G_i = 1\right]}_{\substack{\text{Average treatment effect in} \\ \text{treated population } (\text{ATT})}}
\end{align*} 
\normalsize
\end{itemize}
\end{frame}
...
\begin{figure}
\includegraphics[width = 0.9\linewidth]{trend_stability_plots.pdf}
\vskip1ex
\end{figure} \vfill
\end{itemize}
\vfill
