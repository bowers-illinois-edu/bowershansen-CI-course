...
\date{June 22 2023} % Date, can be changed to a custom date
...
\pause
  ...
  \item[] \indent $y_i(1)$ or $y_i(0)$: Fixed value of the outcome for unit $i$ if it were to receive treatment or control
  \item[] E.g., $y_i(1)$: voter turnout of person $i$ if person $i$ were to receive mail encouraging turnout
  \item[] E.g., $y_i(0)$: voter turnout of person $i$ if person $i$ were \textit{not} to receive mail encouraging turnout
  \end{itemize}
\end{frame}
%------------------------------------------------------------------------
...
\tau_i & = & y_i(1) - y_i(0)
\end{eqnarray*}
\item Other functions of of individual potential outcomes possible, e.g., $\dfrac{y_i(1)}{y_i(0)}$ \pause
...
\begin{frame}
  ...
  \item[] (2) \mh{Complete randomization}: Exactly $n_1$ units are treated and $N - n_1 = n_0$ units are untreated
  \item[] (3) In practice, (1) and (2) are equivalent when we fix $n_1$ by conditioning on its observed value
    \vspace{0.1in}
  \item Under complete or simple (conditioning on observed $n_1$) randomization
$$
\E\left[Z_i\right] = \dfrac{n_1}{N}
$$
  \end{itemize}
\end{frame}
%------------------------------------------------------------------------
...
\begin{frame}
  ...
  \item[] 
    {\footnotesize
      \begin{eqnarray*}
        \hspace{-0.4in} \E\left[\hat{\tau}\left(\bm{Z}, \bm{Y}\right)\right] & = & \E\left[\frac{1}{n_1} \sum_{i=1}^N Z_i Y_i - \frac{1}{n_0} \sum_{i = 1}^N (1 - Z_i) Y_i\right]\\
        \hspace{-0.4in} & = &  \frac{1}{n_1} \sum_{i=1}^N \E\left[Z_i Y_i\right]
                              - \frac{1}{n_0} \sum_{i=1}^N \E\left[(1 - Z_i) Y_i\right] \hspace{0.1in}
                              \mbox{($\because$ Linearity of $\E$)}\\ 
        \hspace{-0.4in} & = &  \frac{1}{n_1} \sum_{i=1}^N \E\left[Z_i y_i(1)\right]
                              - \frac{1}{n_0} \sum_{i=1}^N \E\left[(1 - Z_i) y_i(0) \right] \hspace{0.1in}
                              \mbox{($\because$ Definition of POs)}\\ 
        \hspace{-0.4in} & = &  \frac{1}{n_1} \sum_{i=1}^N y_i(1) \E\left[Z_i\right]
                              - \frac{1}{n_0} \sum_{i=1}^N y_i(0) \E\left[1 - Z_i\right] \hspace{0.1in}
...
\scriptsize
    ...
    $\bm{z}_1$ & $\bm{y}(\bm{0})$ & $\bm{y}(\bm{1})$ & $\bm{y}_1$ \\ \midrule
    1 & ?  & 15  & 15 \\
    1 & ?  & 15  & 15 \\
    0 & 20 & ?   & 20 \\
    0 & 20 & ?   & 20 \\
    ...
    \end{tabular}
     ...
     \hfill
      \begin{tabular}{l|l|l|l|}
    $\bm{z}_{21}$ & $\bm{y}(\bm{0})$ & $\bm{y}(\bm{1})$ & $\bm{y}_{21}$ \\ \midrule
    0 & 10 & ?  & 10 \\
    0 & 15 & ?  & 15 \\
    0 & 20 & ?  & 20 \\
    0 & 20 & ?  & 20 \\
