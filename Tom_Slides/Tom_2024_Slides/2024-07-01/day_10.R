...
pre_post_data <- data.frame(mean_outcome = c(70, 100,
                                             70, 70),
                            Treatment = rep(x = c("Treated group", "Treated group's counterfactual trend"), each = 2),
                            Period = rep(x = c(0, 1), times = 2))
pre_post_data$Treatment <- factor(x = pre_post_data$Treatment,
                                  levels = c("Treated group", "Treated group's counterfactual trend"))

pre_post_plot <- ggplot(data = pre_post_data,
                        mapping = aes(x = Period,
                                      y = mean_outcome,
                                      group = Treatment)) +
  geom_point(data = pre_post_data) +
  geom_line(mapping = aes(linetype = Treatment)) +
  scale_linetype_manual(values = c("solid", "dotted")) +
  xlab("Time") +
  ylab("Mean outcome") +
...
#ggsave(plot = pre_post_plot,
#       file = "figures/pre_post_plot.pdf",
#       width = 6,
#       height = 4,
#       units = "in",
#       dpi = 600)

ITS_data_A <- data.frame(mean_outcome = c(50, 60, 70, 100,
                                          NA, NA, 70, 70,
                                          NA, NA, 70, 80),
                         Treatment = rep(x = c("Treated group",
                                               "Treated group's counterfactual trend A",
                                               "Treated group's counterfactual trend B"), each = 4),
                         Period = rep(x = c(-2, -1, 0, 1), times = 3))
ITS_data_A$Treatment <- factor(x = ITS_data_A$Treatment,
                               levels = c("Treated group",
                                          "Treated group's counterfactual trend A",
                                          "Treated group's counterfactual trend B"))

ITS_plot_A <- ggplot(data = ITS_data_A,
                     mapping = aes(x = Period,
                                   y = mean_outcome,
                                   group = Treatment)) +
  geom_point(data = ITS_data_A) +
  geom_line(mapping = aes(linetype = Treatment)) +
  scale_linetype_manual(values = c("solid", "dotted", "dashed")) +
  xlab("Time") +
  ylab("Mean outcome") +
  scale_x_continuous(limits = c(-2, 1),
                     breaks = -2:1,
                     labels = c("Before", "Before", "Before", "After")) +
...
#ggsave(plot = ITS_plot_A,
#       file = "figures/ITS_plot_A.pdf",
#       width = 6,
#       height = 4,
#       units = "in",
#       dpi = 600)

ITS_data_B <- data.frame(mean_outcome = c(70, 70, 70, 100,
                                          NA, NA, 70, 70),
                         Treatment = rep(x = c("Treated group",
                                               "Treated group's counterfactual trend"), each = 4),
                         Period = rep(x = c(-2, -1, 0, 1), times = 2))
ITS_data_B$Treatment <- factor(x = ITS_data_B$Treatment,
                               levels = c("Treated group",
                                          "Treated group's counterfactual trend"))

ITS_plot_B <- ggplot(data = ITS_data_B,
                     mapping = aes(x = Period,
                                   y = mean_outcome,
                                   group = Treatment)) +
  geom_point(data = ITS_data_B) +
  geom_line(mapping = aes(linetype = Treatment)) +
  scale_linetype_manual(values = c("solid", "dotted", "dashed")) +
  xlab("Time") +
  ylab("Mean outcome") +
...
#ggsave(plot = ITS_plot_B,
#       file = "figures/ITS_plot_B.pdf",
#       width = 6,
#       height = 4,
#       units = "in",
#       dpi = 600)
