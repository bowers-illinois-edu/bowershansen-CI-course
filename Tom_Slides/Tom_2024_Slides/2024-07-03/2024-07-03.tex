...
\begin{itemize} \vfill
\item \textbf{Today}: Fisherian inference about sharp causal effects \vfill
\item \textbf{Friday}: Neymanian inference about weak causal effects \vfill
\end{itemize} \vfill
\end{itemize} \vfill
\end{frame}
%----------------------------------------------------------------
...
\begin{itemize} \vfill
\item $Z_i \sim p_i^{z_i} \left(1 - p_i\right)^{1 - z_i}, i = 1, \dots n$, where $p_i \in (0, 1)$ for all $i$ \vfill
\end{itemize} \vfill
...
\begin{align} 
\Pr\left(\mathbf{Z} = \mathbf{z}\right) & = \prod \limits_{i = 1}^n \pi\left(u_i\right)^{z_i} \left(1 - \pi\left(u_i\right)\right)^{1 - z_i}, \\
p_i = \pi\left(u_i\right) & = \cfrac{\exp\left(\gamma u_i\right)}{1 + \exp\left(\gamma u_i\right)}, \, \gamma \geq 0 \label{eq: sens probs}
\end{align} \vfill
...
\end{itemize} \vfill
\item Algorithm is used in \texttt{sensitivitymw} and \texttt{sensitivitymv} \texttt{R} packages  \vfill
\end{itemize} \vfill
\end{frame}
%----------------------------------------------------------------
\begin{frame}{Asymptotic separability algorithm}
...
\begin{itemize} \vfill
\item We made following matched design: \vfill
\vspace{2em}
\begin{table}[H]
...
\end{itemize} \vfill
\item \citet{rosenbaum2018} offers solution ...
\end{itemize} \vfill
\end{frame}
%----------------------------------------------------------------
