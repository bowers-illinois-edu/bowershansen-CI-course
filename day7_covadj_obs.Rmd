...
## Today
 ...
 1. Agenda:
   - Recap: Modes of statistical inference for causal effects, Learning about
     the causal effect of treatment **receipt** (or a **dose** of treatment) on
     non-random groups of people using an **instrument** (that was created by
     the researcher via randomization).
...
```{r child="child_files_/predictive_inference_in_rcts.Rmd", eval=TRUE}
```
```{r child="child_files_/per_protocol_vs_itt_iv.Rmd", eval=FALSE}
```
