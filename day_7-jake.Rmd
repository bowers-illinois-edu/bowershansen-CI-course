...
## Today
 ...
 1. Agenda:
- Recap: Day 6 on instruments, the problems of per-protocol comparisons versus
as-assigned, and estimation in block-randomized experiments.
...
interpolation, linearity, influential points, parallel slopes, and in general
whether we have "controlled for" enough.
-  Introducing stratification for adjustment via modern matching: changing the
question from "why did you make those groups" to "how to minimize differences
within group"
...
Ok. What about simplifying? When a person wants to know whether we have
"controlled for", say, $x_4$, I suspect they are really asking for this:
...
In this case we can say that we have "held constant" $x_4$. But what is the
**overall estimate** in this case?

Choosing an additive and linear functional form allows us to predict $Y$ for
any given $x$ where the differences in predicted Y relate to differences in x
in a constant way with respect to the other variables. But this an implication
or consequence of the linearity and additivity choice.
...
```{r}
## from propertee
the_design <- obs_spec(Z~unit_of_assignment(id)+block(x4),data=dat)
est3 <- lmitt(Y~Z,specification=the_design,data=dat,weights="ate")
coef(est3)[["Z."]]

ate(the_design,data=dat)
dat$nbwt
...
What if we wanted to adjust for $x_2$? 
...
We have two questions: (1) Did we adjust for $x_2$ enough? and (2) Why did we choose subsets based on $x_2=3$?

## Matching instead of breakpoints
...
in $x_2$. How do make homogeneous comparisons on $x_2$? (Answer: try modern matching)
