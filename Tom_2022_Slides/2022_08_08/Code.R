...
treat_data <- data.frame(score = seq(from = 1600, to = 2400, by = 10))
cutoff <- 2000
treat_data <- mutate(.data = treat_data,
                     treatment = ifelse(test = score < cutoff,
                                        yes = 0,
                                        no = 1))

treatment_plot <- ggplot(data = treat_data,
                         mapping = aes(x = score,
                                       y = treatment)) +
  geom_line(data = filter(.data = treat_data,
                          treatment == 0),
            colour = "red") +
  geom_line(data = filter(.data = treat_data,
                          treatment == 1),
