...
## There are 0 ways for the confounding to be given to 0 of 6 obs
## There are 6 ways for only a single obs to get the confounding 
## And more ways for exactly 2 of the 6 to gave the confounding.
u_combs[,1:10]
...
## For each way to assign treatment (column of Omega)
## there are 64 ways of confounding (using the binary confounding
## representation)
sens_Omega_probs <- sapply(
  X = 1:ncol(u_combs),
  FUN = function(x) {
    sens_probs(
      .Gamma = 2,
...
p_values <- sapply(
  X = 1:ncol(sens_Omega_probs),
  FUN = function(x) {
    sum((null_test_stats >= obs_test_stat) * sens_Omega_probs[, x])
  }
)

## These are the u that **reduce** the p-value.
u_combs[, which.min(p_values)]

anti_con_sens_null_dist_data <- data.frame(
  null_test_stat = rep(x = null_test_stats, times = 2),
...
anti_con_sens_null_dist_z_8 <- ggplot(
  data = anti_con_sens_null_dist_data,
  mapping = aes(
    x = null_test_stat,
    y = prob
  )
...
ggsave(
  plot = anti_con_sens_null_dist_z_8,
...
sens_null_dist_data <- data.frame(
  null_test_stat = rep(x = null_test_stats, times = 2),
...
sens_null_dist_z_8 <- ggplot(
  data = sens_null_dist_data,
  mapping = aes(
    x = null_test_stat,
    y = prob
  )
...
min_p_values <- rep(x = NA, times = length(Gammas))

for (i in 1:length(sens_Omega_probs)) {
  sens_Omega_probs[[i]] <- sapply(
    X = 1:ncol(u_combs),
    FUN = function(x) {
      sens_probs(
        .Gamma = i,
  ...
  p_values[[i]] <- sapply(
    X = 1:ncol(sens_Omega_probs[[i]]),
    FUN = function(x) {
      sum((null_test_stats >= obs_test_stat) * sens_Omega_probs[[i]][, x])
    }
  )

  max_p_values[i] <- max(p_values[[i]])
  min_p_values[i] <- min(p_values[[i]])
}

sens_plot_data <- data.frame(
  Gamma = Gammas,
  max_p_value = max_p_values,
  min_p_value = min_p_values
)
