...
opts_chunk$set(tidy=TRUE,echo=TRUE,results='markup',strip.white=TRUE,fig.path='figs/fig',cache=FALSE,highlight=TRUE,width.cutoff=100,
	       size='footnotesize',out.width='1.2\\textwidth',message=FALSE,warning=FALSE,comment=NA)
...
Notice that the randomization distributions looks a little strange; that is,
both randomization distributions are very different from the normal
distribution to which we are accustomed. This is important. Nothing so far has
invoked any claim about the shape of the randomization distribution, such as
its normality if the finite population central limit theorem obtains
\citep{hajek1960}. What matters (in terms of unbiasedness) is whether the mean
of the randomization distribution---i.e., the mean of all the estimated
difference in means---is equal to the true average treatment effect.\footnote{
	Notice that we do not have simulated results that are \emph{exactly}
...
psDist2<-match_on(brglm1,data=meddat)

meddat$pScore2<-predict(brglm1)
...
fm1 <- fullmatch(mahalDist + caliper(psDist2,3),data=meddat,min.controls=.5)
summary(fm1,propensity.model=brglm1,data=meddat)

meddat[names(fm1),"fm1"] <- fm1

meddat %<>% filter(!is.na(fm1)) %>% arrange(desc(fm1)) %>% mutate(fm1 = as.numeric(fm1))
...
obs_ate <- coef(lm(HomRate08 ~ nhTrt + as.factor(fm1), data = meddat))[["nhTrt"]]
