...
\vspace{-5mm}
\begin{itemize}\itemsep1pt
\item Is the variance of the randomization distribution tighter when treatment is assigned within blocks or when treatment is \textit{not} assigned within blocks? If so, why?
