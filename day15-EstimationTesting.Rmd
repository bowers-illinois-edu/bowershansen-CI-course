...
as.matrix(matchdist)[1:4,1:6]
matchdistPen[1:4,1:6]

fm2 <- fullmatch(matchdistPen,data=meddat,min.controls=.5,mean.controls=22/23)
...
plot(graph_from_adjacency_matrix(blah2, mode = "undirected", diag = FALSE),
  vertex.color = c("white", "green")[meddat$nhTrt + 1], main = "Penalties,Min Ctrls=.5, Max Ctrls=Inf"
)
```
...
## Test the function
findbalance(c(psCal,mhCal),thedat=meddat,psdist=psdist,mhdist=mhdist)
...
interestingDesigns<- (resNoNA["d2p",]>.5 & resNoNA["n",]>=40 &
    resNoNA["maxHR03diff",]<10 & resNoNA["effn",] > 12)
...
mom_tols <- round(absstddif(mom_covs,z,.5),2)
