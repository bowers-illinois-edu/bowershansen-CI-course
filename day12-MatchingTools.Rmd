...
How can we argue that we have adequate adjustment for background covariates in observational studies?
  ...
  - Assessing stratifications using the block-randomized design as a standard
    of comparison. (What does this mean in practice? What does  `balanceTest` do?
    How does `balanceTest` adjust differences of means for the stratifications?)

## Matching on the Mahalanobis Distance: Setup

Here using the ordinary Mahalanobis distance.
...
```{r xbmh, echo=TRUE}
xbMh <- balanceTest(nhTrt ~ nhAboveHS + nhPopD + HomRate03+strata(fmMh),
   data = meddat
)
xbMh$overall
...
```{r echo=TRUE}
theglm <- arm::bayesglm(nhTrt ~ nhAboveHS + nhPopD + nhPopD.NA + HomRate03, data = newdat, family=binomial(link="logit"))
psdist <- match_on(theglm, data = meddat)
