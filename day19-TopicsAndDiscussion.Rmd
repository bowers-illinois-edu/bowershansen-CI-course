...
categorical outcome between units. For example:
...
Consider Mahalanobis distance on all past outcomes? (See Nielson on "Matching
...
```{r}
 knitr::include_graphics(here("images", "interference-example-2.pdf"))
```

Introducing the \textbf{uniformity trial} $\equiv \by_{i,0000}$ (Rosenbaum,
2007).
...
\includegraphics[width=.95\textwidth]{images/fishersutvaModel.pdf}

## Learning about a causal model from an experiment on a network

\includegraphics[width=.6\textwidth]{images/ssr2d.pdf}
...
In fact *any function* that produces vectors of $\yu$ could be used to represent these kinds of causal models.

So: why not an agent-based model?
