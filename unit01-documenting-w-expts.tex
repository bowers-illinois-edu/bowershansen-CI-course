...
Write $z_{i} = \left\{
  \begin{array}{lr}
    1, & i\sim \mathrm{dolphins},\\
    0, & i\sim \mathrm{no\, dolphin};\\
  \end{array}
\right. \, \mathbf{z}= (z_{1}, \ldots, z_{30}).$

The \textit{hypothesis of absolutely no effect} says the under any
other $\mathbf{z}$ each cured
participant would have been cured, and each non-cured participant
would not have been cured..

\footnotetext[1]{Antonioli, C. and
 Reveley, M.A. (2005)  ``Randomised controlled trial of animal
 facilitated therapy with dolphins in the treatment of depression.''
 \textit{British Medical Journal} \textbf{331} (7527) 1231--4; see
 also \href{http://www.rossmanchance.com/applets/Dolphins/Dolphins.html}{rossmanchance.com Applet}. }
\end{frame}
\note{
...
\begin{frame}{Interference\footnote{(instructor agenda alert!)}}
  \begin{itemize}
    \item Intuitively, my treatment may affect your response
  \item If there is interference, describing the true state of affairs calls for more elaborate notation than $(y_{T1}, y_{C1}), (y_{T2}, y_{C2}), \ldots, (y_{Tn}, y_{Cn}).$
