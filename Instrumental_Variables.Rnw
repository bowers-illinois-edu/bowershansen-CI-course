...
\author{Jake Bowers, Ben Hansen, and Tom Leavitt}
\date{Summer 2015}
\maketitle

\doublespacing

These data are taken from Finkelstein et al, 2012, ``The Oregon Health Insurance Experiment: Evidence from the First Year,'' Quarterly Journal of Economics, 127 (3): 1057 - 1106. Oregon conducted a lottery to offer individuals the opportunity to sign up for Medicaid. Winning the lottery, however, only granted individuals the opportunity to apply for Medicaid, and the applications were accepted only if the applicants met the eligibility requirements.
...
OHIE[col_nums] <- sapply(OHIE[col_nums], as.integer) - 1
...
cace_3 <- coef(lm(baddays_phys_12m ~ predictions_ittd, data = OHIE_filtered,subset=numhh_list=='signed self up'))[2]
...
thecaces<-sapply(stratified_IV_fit,function(obj){ coef(obj)[2,1] })
...
all.equal(thecaces,c(caceHH1[[1]],caceHH2[[1]],caceHH3[[1]]))
...
correlation between Z and Y because D, which comes before Y, cannot be a descendent of Y.) Let's try that:

<<>>=

treat_rec_subset <- c(0, 1)

lapply(treat_rec_subset, function(x) summary(lm(baddays_phys_12m ~ treatment, data = OHIE_filtered, subset = ohp_all_ever_admin == x)))

@

Here is how the standard errors are being calculated:

<<>>=

# This is for the LATE/CACE

iv_fit <- ivreg(baddays_phys_12m ~ ohp_all_ever_admin | treatment, data = OHIE_filtered)
summary(iv_fit)
N <- nrow(OHIE_filtered)
X_mat <- cbind(rep(1, N), OHIE_filtered$ohp_all_ever_admin)
Z_mat <- cbind(rep(1, N), OHIE_filtered$treatment)
residuals <- OHIE_filtered$baddays_phys_12m - X_mat %*% coef(iv_fit)
...
The denominator of the CACE estimator will simply be the proportion of $d = 1$s among the $z = 1$s minus the proportion of $d = 1$s among the $z = 0$s. That is, $\frac{(1 + 1 + 1 + 1)}{4} - \frac{(0 + 1 + 1 + 0)}{4} = \frac{1}{2}$. In the treatment group, $z = 1$, there is no non-compliance; thus, subjects can be only compliers or always-takers. There is non-compliance in the control group, $z = 0$, and those non-compliers could be either always-takers or defiers.
...
And if the 1s in the control group are always-takers, then the regression of $\mathbf{d}$ on $\mathbf{z}$ gives us the proportion of compliers among the treated group which, due to $Z {\perp\!\!\!\perp} D$ is an unbiased estimate of the proportion of compliers in the entire pool of subjects.

Essentially what we are doing is using the proportion of always takers in the control group to estimate the proportion of always takers in the treatment group, which then makes the regression of $\mathbf{d}$ on $\mathbf{z}$ an estimate of the proportion of compliers in the treatment group, which is an estimate of the proportion of compliers in the pool of experimental subjects.
