...
author-meta: Jake Bowers, Tom Leavitt
bibliography:
...
- A: Many approaches
  1. Randomize $Z$;
  2. Randomize an instrument for $Z$ (or $D$);
...
## General Introduction and Setup {.shrink}
...
```{r echo=FALSE, out.width=".7\\textwidth", warning=FALSE}
...
hypothesis is at most $\alpha$" \citep[p. 185]{hansensales2015}. This is the Sequential Intersection Union Principle (SIUP) of hypothesis testing.
\smallskip
...
## The Pct diffs in the first, and widest, window of
c(lower_bound[1], upper_bound[1])
...
chi_squared_balance <- function(i,
                                running_var,
                                bal_fmla,
                                thelower_bound = lower_bound,
                                theupper_bound = upper_bound,
                                data) {
  # Preliminaries
  ...
  window_dat <- data %>% filter(running_var > thelower_bound[i] & running_var < theupper_bound[i])
  ...
  bands <- cbind(ess, p_value, lb=thelower_bound[i], ub=theupper_bound[i])
...
Now use the function (here using parallel processing)
...
# chi_squared_balance(1, running_var = rdd_data$DifDPct, bal_fmla = bal_fmla, data = rdd_data)

res0 <- future_sapply(is,
  chi_squared_balance,
...
band_df <- data.frame(t(res0)) %>% rename( ess = X1, p_value = X2, lb=X3,ub=X4)
...
```{r}
band_df$limits <- paste(round(band_df$lb, 1), round(band_df$ub, 1), sep = ",")
...
```{r}
band_df[47, c("ess", "p_value", "lb", "ub")]
```

Idea: use power analysis prior to outcome analysis to choose bandwidth. Sample size and/or variance in outcome required for power and then use p-value arising from that.

## Optimal Bandwidth Selection {.allowframebreaks}

The data within the selected window:
...
running variable (and covariates that relate to the running variable). Thus, if
$R$ relates to $(y_{c}, y_{t})$, through a mechanism other than $Z \given R \in
W_0$, then the exlcusion restriction is violated.
...
What can we do if the running variable, $R$, confounds the relationship between
$Z$ and $Y$ within the window? One approach is to model potential outcomes as a
function of the running variable, and then to "de-trend" (or "transform'') the
outcome variable and to subsequently argue that we have "residual
ignorability." (See Hansen and Sales).

For example, we could do the following:
```{r}
## Estimate a linear trend
tmp_lm <- lm(DPctNxt ~ DifDPct, data = rdd_data47)
## Estimate a non-linear trend resistent to outliers
## We could look for an optimal bandwidth for this smoother by minimized SSE or something.
tmp_loess <- loess(DPctNxt ~ DifDPct, data = rdd_data47, family="symmetric", deg=1, span=1/3)
rdd_data47  <- rdd_data47 %>% mutate(resid_DPctNxt = resid(tmp_lm),
resid_DPctNxt_loess=resid(tmp_loess))
```
...
## Outcome Analysis on Detrended Outcome Variable
...
lm_robust(formula = resid_DPctNxt_loess ~ DemWin, data = rdd_data47)
```

@saleshansen2020, however, propose robust regression, which is less sensitive to violations of the regression model's assumptions --- although still removing linear trend here. And notice that we can test hypotheses about the effect caused by crossing the discontinuity using the detrended variable as well.
...
We could also match across the threshold to either (1) Just treat this as a
natural experiment (i.e. no exclusion restriction, no special assumptions about
$R$ and $Z$) or (2) Strengthen arguments in favor of exclusion restriction.

```{r}

mdist <- match_on(bal_fmla, data = rdd_data47)
pm1 <- fullmatch(mdist, data = rdd_data47, remove.unmatchables = TRUE)
summary(pm1)
```
...
rdd_data_cp <- dplyr::select(.data = rdd_data, DifDPct, DPctNxt, DemWin) %>%
  mutate(cp = factor(ifelse(test = DifDPct < 0, yes = "Below Cutpoint",
    no = "Above Cutpoint"
  )))

rdd_data_cp$cp<- fct_relevel(rdd_data_cp$cp,c("Below Cutpoint","Above Cutpoint"))
...
ggplot(
  ...
  geom_smooth(method = lm, se = FALSE) +
  geom_smooth(method = rlm, se = FALSE, colour = "black") +
  geom_smooth(method = loess, se = FALSE, colour = "red", method.args = list(deg = 1, span = 1 / 3, family = "symmetric")) +
  theme(
...
We could also do the same thing with LOESS regression (here, choosing span and degree arbitrarily)
...
We could also use a $p$th order polynomial although this is not a good idea because it is a global fit. For example:
...
This next calculates a bandwidth and then does what we just did above (all in one step) using the `rdrobust` package:
...
\citet[647]{cattaneotitiunikvazquez-bare2017} view the potential outcomes as a
random sample from an infinite superpopulation. This differs this course's
emphasis on finite population, design-based causal inference. An interesting
moment to think about whether and how this difference in foundational
assumptions matters or not in a case like this.
