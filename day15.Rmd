...
results1<-sapply(seq(maxmhdist,minmhdist,length=100),function(thecal){
                   matchAndBalance(thecal,balfmla,distmat=mhdist)})

apply(results1,1,summary)

results1[,results1["d2p",]>.2]
...
findbalance<-function(x){
	##message(paste(x,collapse=" "))
	thefm<-try(fullmatch(mhdist+caliper(mhdist,x[1])+caliper(psdist,x[3]),data=meddat,tol=.00001,min.controls=1))
...
set.seed(123455)
system.time({
	results<-replicate(5000,findbalance(c(runif(1,minpsdist,maxpsdist),
