...
Note variance/bias tradeoff. Covariate balance has to do with bias and extrapolation (i.e. confounding).
...
```{r eval=TRUE,echo=FALSE}
interestingDesigns<- (resNoNA["d2p",]>.1 & resNoNA["n",]>=10 &
		      resNoNA["maxHR03diff",]<=2 & resNoNA["effn",] > 6)
...
fm4<-fullmatch(psdist+caliper(psdist,candDesigns["x1",2])
	       +caliper(mhdist,candDesigns["x2",2]),data=meddat,tol=.00001,min.controls=1)
...
matchAndBalance(results3$par[1],balfmla=balfmla,distmat=psdist)
```
...
This means we have to *define* our estimands in weighted terms (because different blocks provide different amounts of information). And our null reference distribution arise from block-randomization. One version weights by size of set (often just called the ATE), another by number of treated (the ATT), another by the precision of the block-level treatment effect estimate.
