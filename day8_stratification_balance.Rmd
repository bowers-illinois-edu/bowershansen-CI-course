...
Say we find the plane that best fits the data
...
g1 <- ggplot(dat,aes(x=resid_Z_x1,y=resid_Y_x1))+geom_point()+geom_smooth(method="lm",se=FALSE)
print(g1)
```
...
with(dat,plot3d(x=x1,z=Z,y=Y,type="s"))
planes3d(b=coef(lm2)[["x1"]],a=coef(lm2)[["Z"]],c=1,d=coef(lm2)[["(Intercept)"]],alpha=.5)

uniq_x1 <- sort(unique(dat$x1))
pdat <- expand.grid(x1=uniq_x1,Z=c(0,1))
pdat$fit <- predict(lm2,newdata=pdat)
pdat_mat <- matrix(pdat$fit,ncol=2,nrow=length(uniq_x1))
surface3d(x=uniq_x1,y=c(0,1),z=pdat_mat)

region <- expand.grid(x1=sort(unique(dat$x1)),Z=c(0,1))
