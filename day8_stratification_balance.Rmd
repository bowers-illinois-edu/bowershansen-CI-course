...
specification, readers wonder: (1) is this just the best result out of
thousands? Are most results like this? Or was this cherry picked? (2) how many
$p$-values were looked at before reporting this one with $p < .05$? (Recall
...
error (a false positive result) or that it truly would be very rare to see the
observed result under the null and that the risk of the false positive result
is less than or equal to .05.  But a $p <  .05$ after 100 tests is virtually
certain to occur (with false positive prob of .99) when $H_0$ is true so we
should not reject the null even if the $p$ is small.
