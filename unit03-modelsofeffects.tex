...
\item Parametric $t$-tests may have sizes larger than their levels. Permutation $t$-tests will maintain size.
\item For both tests, \textit{power} suffers.
...
dependence assumptions.   For instance, using random
 ...
 \begin{enumerate}[<+->]
 \item Let
   $y(T)=$ neighborhood homicide rate in year $T$.  We test models of
   effects of the form
$$H_{0}: y_{t}(T) = \left\{ \begin{array}{lr}\exp(T \cdot r/4)
                              y_{c}(T),& T>0 \\
y_{c}(T),& T\leq 0\end{array} \right.$$ 
where  $r \leq 0$.
 \item We then fit a random effects model of this form:
\begin{semiverbatim}
 lmer(Count \textasciitilde\  year + (year+1|nh) +\ \pause
            \alert<@+>{offset}( nhTrt*(yr>2004)*(r/4)*(yr-2004) ),
          family=poisson, data=homd)
\end{semiverbatim}
\item This fits 2 random effects, a slope and an intercept, for
  each neighborhood.  To test $H_{0}$, I calculated a test statistic comparing $t$ vs $c$
  differences in fitted slopes, then compared it to its rerandomization distribution.
 \end{enumerate}
