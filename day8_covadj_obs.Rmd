...
## Today
 ...
 1. Agenda:
     - Recap: Adding quick demo of Bayesian predictive approach to causal inference.
...
The whole point of covariance adjustment in experiments is precision, not adjusting away potential confounders.
...
```{r plotcooks, outwidth=".8\\textwidth"}
...
dat <- dat %>% group_by(x4) %>% mutate(nb = n(),
    mb = sum(Z),
    ateb = mean(Y[Z == 1]) - mean(Y[Z == 0]),
    prob_trt = mean(Z),
