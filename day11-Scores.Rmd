...
## Today
  ...
  1. Agenda:  Continue to think about an adjustment strategy to enhance
     counterfactual causal interpretations in non-randomized studies:
      - Decide to adjust $\rightarrow$ "fair comparison" $\rightarrow$ stratification (ex. `fullmatch`) $\rightarrow$ evaluation of the stratification (ex. `xBalance`)
      - How to do this with one variable and/or more than one variable.
 2. Reading for tomorrow and this week: DOS 8--9, 13 and
...
The goal of optimal matching: collect observations into strata which minimize
the within strata differences (calculate the absolute difference between
treated and controls within strata, sum across strata, minimize this quantity).
**Optimal** in the sense of minimization.  (**Full matching** has the
additional goal of not excluding any observations by default.)
