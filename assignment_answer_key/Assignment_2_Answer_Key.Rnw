...
acorn <- read.csv2("acorn03.csv", sep = ",") 

acorn_indiv <- data.frame(unit = acorn$unit,
                          size = acorn$size,
                          z = acorn$z,
                          contact = as.numeric(acorn$contact) * acorn$size,
                          vote = as.numeric(acorn$vote03) * acorn$size)
...
ITT_hat <- (sum(acorn_indiv$vote[acorn_indiv$z == 1]) / sum(acorn_indiv$size[acorn_indiv$z == 1])) -
  (sum(acorn_indiv$vote[acorn_indiv$z == 0]) / sum(acorn_indiv$size[acorn_indiv$z == 0]))
...
ITT_D_hat <- (sum(acorn_indiv$contact[acorn_indiv$z == 1]) / sum(acorn_indiv$size[acorn_indiv$z == 1])) -
  (sum(acorn_indiv$contact[acorn_indiv$z == 0]) / sum(acorn_indiv$size[acorn_indiv$z == 0]))

CACE_hat <- ITT_hat / ITT_D_hat
