...
\begin{align*}
\Pr\left(Z_i = 1\right) & = \cfrac{\exp\left(\kappa\left(\mathbf{x}_i\right) + \gamma u_i\right)}{1 + \exp\left(\kappa\left(\mathbf{x}_i\right) + \gamma u_i\right)},
\end{align*}
where $\kappa\left(\cdot\right)$ is an unknown function and $\gamma$ is an unknown parameter.
