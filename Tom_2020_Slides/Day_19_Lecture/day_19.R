...
DID_data$Treatment <- factor(x = ID_data$Treatment,
                             levels = c("Treated", "Control", "Treated counterfactual trend"))

DID_plot <- ggplot(data = DID_data,
...
DID_data_no_pt <- data.frame(mean_outcome = c(NA, NA, 60, 90,
                             ...
                             Treatment = rep(x = c("Treated counterfactual trend", "Treated", "Control"), each = 4),
                             Period = rep(x = c(-1, 0, 1, 2), times = 3))
DID_data_no_pt$Treatment <- factor(x = DID_data_no_pt$Treatment,
                             levels = c("Treated", "Control", "Treated counterfactual trend"))

DID_no_pt_plot <- ggplot(data = DID_data_no_pt,
                   mapping = aes(x = Period,
                                 y = mean_outcome,
                                 group = Treatment)) +
  geom_point(data = subset(x = DID_data_no_pt,
                           subset = Treatment %in% c("Treated",
                                                     "Control"))) +
  geom_line(mapping = aes(linetype = Treatment)) +
  scale_linetype_manual(values = c("solid", "dotted", "dashed")) +
