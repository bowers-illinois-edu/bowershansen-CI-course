...
## Today
  ...
  1. Agenda: The problem of covariance adjustment to reduce "bias"/
     ...
     form?** A simple approach: stratification on one categorical variable
     followed by a comparison of this stratification with one or more
     **standards** of unconfounded designs. A more complex approach: find sets
...
# Strategies for Causal Inferencr
...
```{r echo=TRUE, out.width=".8\\textwidth"}
## Notice that the distribution of d2_dist is not that close to the
## chi-squared_distribution in this case with N=28
par(mfrow = c(1, 2))
...
<https://www.menti.com/cxvdb8ecmj>
...
xbate1$results[, c("nhTrt=1", "nhTrt=0", "adj.diff"), ]
```
...
xbHS1$overall
xbHS1$results[1, c("nhTrt=1", "nhTrt=0", "adj.diff", "std.diff", "z", "p"), ] ## the covariate specific z-test
```
...
$\beta_1$ in equation \@ref(eq:olsbiv) is the same as the difference of means
