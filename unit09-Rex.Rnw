...
\item Some people would say that 1.5 is a good estimate of the average
 ...
 \item  In 1923, \citet{neyman:1923} proposed (1) that we decide to stop focusing on
   ...
   $\hat{\bar{\tau}}=(1/m)\sum_{i=1}^n Z_i Y_i - (1/(n-m))\sum_{i=1}^n (1-Z_i)
   Y_i$ was a good \textbf{estimator} of $\bar{\tau}$ (where $Y_i$ is observed
   outcomes and relates to potential outcomes via $Y_i=Z_i y_{1i}+(1-Z_i)
   y_{0i}$, $m$ is number of treated observations and $n$ is total number of
...
y0<-news.df$r # we could have also use y0<-rnorm(48) or something else
