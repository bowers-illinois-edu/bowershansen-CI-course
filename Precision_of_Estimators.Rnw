...
\begin{center}
\begin{align*}
Var(\bar{y}_t - \bar{y}_c) & = \text{Var}\left(n_t^{-1} Z^{\prime} y_t \right) + \text{Var}\left(n_c^{-1} (1 - Z^{\prime}) y_c \right) \\
\\
...
\hat{\sigma}_{\hat{ate}}|\rho_{tc} = 1 = \sqrt{\frac{\sigma_{yc}^2}{n_c} + \frac{\sigma_{yt}^2}{n_t}}
\end{equation}
\end{center}
...
lm_fm_1 <- lm(HomRate08 ~ nhTrt + fm1, data = meddat)
...
new_ci_constant_additive(y = meddat$HomRate08,
       z = meddat$nhTrt,
       fm = meddat$fm1)

ci_coverage_constant_additive <- replicate(10^4, new_ci_constant_additive(y = meddat$HomRate08,
                                      z = meddat$nhTrt,
                                      fm = meddat$fm1))
...
meddat %<>% mutate(yc = ifelse(test = nhTrt == 0,
                               yes = HomRate08,
                               no = NA),
...
true_ate <- meddat %$% {mean(yt) - mean(yc)}
...
new_ci_non_cons_add(yc = meddat$yc,
                          yt = meddat$yt,
                          z = meddat$nhTrt,
                          fm = meddat$fm1)

ci_coverage_non_cons_add <- replicate(10^4, new_ci_non_cons_add(yc = meddat$yc,
                                                   yt = meddat$yt,
                                                   z = meddat$nhTrt,
                                                   fm = meddat$fm1))
