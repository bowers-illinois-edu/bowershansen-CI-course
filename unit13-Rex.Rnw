...
<<dosens, results="hide">>=
install.packages("http://jakebowers.org/Matching/sensitivitymv_1.3.tar.gz",
		 type="source",repos=NULL, method="libcurl") ## or method="curl" if needed
...
respmat<-with(meddat[matched(fm4),],fullmatch2sensmat(HomRate0803,nhTrt,fm4))

## This is a hack. It involves adding one control unit.
## It appears that we have to have at least two sets of the
## same size to make this code work.
## Here I'm trying to set some bounds on the answers that we get with
## different values for the difference between the treated unit and the new
## fake control
## respmat[4,4]<-max(respmat,na.rm=TRUE)
## respmat[4,4]<-0
## respmat[4,4]<-min(respmat,na.rm=TRUE)

## Alternatively install a version of the package that I edited from the
## website http://jakebowers.org/Matching

## install.packages("http://jakebowers.org/Matching/sensitivitymv_1.3.tar.gz", repos=NULL, method="libcurl")
...
If we had fixed size sets (i.e. all 1:1 or all 1:2...), we can also look at an example involving
...
<<>>=
## Here, remove the one set that is not a pair:
meddat$pmFake<-meddat$fm4
meddat$pmFake[meddat$fm4 %in% names(table(meddat$fm4)[table(meddat$fm4)!=2])]<-NA
...
respmatPm<-with(droplevels(meddat[matched(meddat$pmFake),]),fullmatch2sensmat(HomRate0803,nhTrt,pmFake))
