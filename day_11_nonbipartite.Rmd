...
## Data previously fetched from http://jakebowers.org/Data/meddat.rda (now returns 404); load local copy.
load(here::here("data", "meddat.rda"))
...
## nhClass is stored as an ordered factor; treat it as numeric for the Mahalanobis distance.
covmat$nhClass <- as.numeric(covmat$nhClass)
...
library(designmatch)
library(highs)
## We use the open-source HiGHS solver, which needs no license. designmatch (>= 0.5.5,
## on CRAN) supports it natively via the solver name "highs". For very large problems a
## commercial solver such as Gurobi can be faster, but it requires a license.
