...
fmMh <- fullmatch(psdist + caliper(psdist,4) + caliper(absdist,2)
		  + caliper(mhdist,12), min.controls=1/3, # default
...
What are we estimating? Most people would say ACE=$\bar{y}_1 - \bar{y}_0$. What estimator estimates this without bias?


```{r echo=TRUE}
datNoNA[names(fmMh),"fmMh"] <- fmMh
datB <- datNoNA %>% filter(!is.na(fmMh)) %>% group_by(fmMh) %>%
  summarise(Y=mean(HomRate08[nhTrt==1])-mean(HomRate08[nhTrt==0]),
            nb=n(),
	    nbwt= unique(nb/nrow(datNoNA)),
            nTb = sum(nhTrt),
            nCb = sum(1-nhTrt),
	    pb = mean(nhTrt),
	    pbwt = pb * (1-pb),
...
wdat <- datNoNA %>% filter(!is.na(fmMh)) %>% group_by(fmMh) %>% mutate(pb=mean(nhTrt),
                                               nbwt=nhTrt/pb + (1-nhTrt)/(1-pb),
                                               gghbwt= 2*( n()/nrow(datNoNA) )*(pb*(1-pb)), ## GG version,
                                               nb = n(),
                                               nTb = sum(nhTrt),
                                               nCb = nb - nTb,
