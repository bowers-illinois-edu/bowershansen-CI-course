...
To use a development version of the RItools or optmatch packages, see
their README files,
\url{https://github.com/markmfredrickson/RItools/tree/master} or \url{https://github.com/markmfredrickson/optmatch/tree/master}
...
<<>>=
simD = replicate(1000, coef(lm(yc.moe0~sample(z), data=acorn.e))[2])

@ 

(Note: the \texttt{mosaic} package provides a nifty alternative to \texttt{replicate()}:
<<eval=FALSE>>=
simD = do(1000) * coef(lm(yc~sample(z), data=acorn.e))[2]
@ 
...
simD = replicate(1000, coef(lm(rank(yc.moe0)~sample(z), data=acorn.e))[2])
@ 
...
simD = replicate(1000, coef(rlm(yc.moe0~sample(z), data=acorn.e))[2])
@ 
...
simD <-  replicate(1000, coef(lm(yc.t.moe0~sample(z), data=acorn.e))[2])
@ 
...
using the Mosaic package's \texttt{shuffle()} function instead of base
R's \texttt{sample()}:
<<eval=FALSE>>=
library(mosaic)
simD = replicate(1000, coef(lm(rank(yc.moe0)~shuffle(z, groups=pairs), data=acorn.e))[2])
@ 
Without a \texttt{groups}
argument, \texttt{shuffle()} would have done the same as
\texttt{sample()}, but with the groups argument permutations are
performed within pairs (specifically, within levels of the factor
variable \texttt{pairs}). Try it yourself at the command line:
<<eval=FALSE>>=
...
@ 
Using \texttt{xBalance} to conduct several of these tests at once:
<<>>=
xBalance(z ~ e0 + e1 + e2, 
