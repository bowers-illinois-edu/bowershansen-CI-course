...
set up for this course.  This will require a few steps. First, create
a \texttt{lib} subdirectory within your working directory.   Next,
...
If you've done that, then load the RItools package using
<<>>=
unloadNamespace(c('optmatch', 'RItools')) # as optmatch may have implicitly loaded the other version
library('RItools', lib.loc='./lib')
library('optmatch')
@ 

If you're instead working with an RItools that you installed the same
way you installed the other packages we've mentioned, then do
<<eval=FALSE>>=
library('RItools')
@ 
...
coef(lm(r0~z, data=transform(acorn.e, z=sample(z))))["sample(z)"]
simD = replicate(1000, coef(lm(r0~z, 
    data=transform(acorn.e, z=sample(z))))["sample(z)"])
