...
\subsection{Differences of simple means}
...
coef(lm(yc_moe0~z, data=acorn_e))["z"] # are we extracting the right coeff?
@ 
...
One and two-sided p-values can be calculated as follows:
<<results="hide">>=
mean(simD>=actualD)
...
As long as our test statistic is the difference of simple means, we can also use the large-sample test based on Neyman's asymptotically conservative standard error (as an alternative to the simulation method).  In this example, we'd be hoping that our sample is just large enough for validity of the test. 
...
Since we're looking at differences of simple means, our statistic is also an unbiased estimator of the precinct-level average treatment effect, i.e. the simple mean of each precinct's difference between its potential responses to treatment and control. 
...
For example, here is a difference of weighted means with covariance
